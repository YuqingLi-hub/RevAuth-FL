...
class Agent():
    def __init__(self, id, args, train_dataset=None, data_idxs=None, mask=None, backdoor_train_dataset=None,initial_alpha=1):
        self.id = id
        self.args = args
        self.error = 0
    ...
    def local_train(self, global_model, criterion, round=None, temparature=10, alpha=0.3, global_mask= None, neurotoxin_mask =None, updates_dict =None,masks=None,delta=None,w_alpha=None,k=None):
        ...
        if masks is not None:
            ...
            print(f'{self.update.shape}, {self.previous_global_model_params.shape}') if hasattr(self, 'update') else None
            print(f'agent {self.id} has similarity {torch.cosine_similarity(self.update.reshape(1,-1),self.previous_global_model_params.reshape(1,-1))} at round {round}') if hasattr(self, 'update') else None
            ##############################################
            self.alpha = self.initial_alpha*abs(self.update.mean()) if hasattr(self, 'update') else self.initial_alpha
        ...
        with torch.no_grad():
            ...
            if masks is not None:
                ...
                update_param_w = utils.embedding_watermark_on_position(
                    masks, _user_param, qim, self.m, alpha=alpha,k=k
                )
